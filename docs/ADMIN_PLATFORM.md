...
Auth is not wired; pages use dummy data. Keep UI on shadcn shell tokens for consistency with `AppShell` / login.
...
7. Soft-lock expired/suspended for a few days (view-only), then hard lock.
...
| Database | Neon | Free 0.5 GB; then usage-based |
| Email | Brevo | Free ~300/day; Starter ₹562.50/mo for 5k |
| Images | ImageKit | Free 20 GB bandwidth / 3 GB storage; Paid $9/mo + |
| Auth | **TBD** | Better Auth (self-hosted in our DB) vs Clerk (hosted; free to 50k MAU with Clerk branding) |
| Logging | Better Stack | Free 3 GB / 3 days; Paid $25 / 40 GB metrics |

## Soft locks / tenancy (implementation notes)

- Soft-lock window: view-only for a few days after expiry/suspension; then block generate/export.
