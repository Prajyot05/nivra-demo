...
```sh
npm i
npm test          # finance + calculate-dispatch fixture tests
npm run dev       # http://localhost:3000
```

- Goal SIP Planner (wired): `/`
- Unified Goal Planner (wired): `/goals` — 6 modes
- Investment Growth (wired): `/growth` — SIP / Step-up / Lumpsum / Periodic
- Child Education Planner (wired): `/education`
...
| `packages/ui` | `CalculatorPage`, inputs (`MoneyInput`, `PercentInput`, `YearInput`, `SelectInput`), `ResultCard`, `ScheduleTable`, charts per [`docs/charts.md`](docs/charts.md), INR formatters | Yash. Prajyot **reuses**, does not fork. |
...
src/app/<product>/page.tsx    UI via @nivra/ui only; charts from docs/charts.md
```
